...
class RAGSystem:
    def __init__(self, retriever, generator_model):
    ...
    def generate_answer(self, query, chunks: List[Dict], top_k=3, temperature=1.0, pre_prompt="You can only answer questions about the provided context. If you know the answer but it is not based in the provided context, don't provide the answer, just state the answer is not in the context provided."):
        ...
        generated_answer = self.generator_model(question=query, context=context, max_answer_len=128, temperature=temperature)
...
def load_models():
    ...
    generator_model = pipeline('question-answering', model='deepset/roberta-base-squad2', device=0)
    return retriever, generator_model
...
retriever, generator_model = load_models()
rag_system = RAGSystem(DenseRetriever(retriever), generator_model)

# Streamlit interface
st.title("RAG System for Question Answering")
